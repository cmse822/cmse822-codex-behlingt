...
int main () {
    ...
    while(ctime < PROBLEM::TIMETARGET){
        ...
        for(int ix{0}; ix < PROBLEM::Nx; ix++){
            ...
            else if (ix == 1) {derivF = backwardDifference(fieldF[ix], fieldF[ix-1], fieldF[ix-1], PROBLEM::DELTAX);}
            else {derivF = backwardDifference(fieldF[ix], fieldF[ix-1], fieldF[ix-2], PROBLEM::DELTAX);}
#else
            //Determine BC behavior;
            if(ix == 0) {derivF = {0,0,0};}
        ...
        for(int ix{0}; ix < PROBLEM::Nx; ix++){
            ...
            else if(ix == PROBLEM::Nx-2) {derivFbar = forwardDifference(fieldFbar[ix], fieldFbar[ix+1], fieldFbar[ix+1], PROBLEM::DELTAX);}
            else {derivFbar = forwardDifference(fieldFbar[ix], fieldFbar[ix+1], fieldFbar[ix+2], PROBLEM::DELTAX);}
#else
            if(ix == PROBLEM::Nx-1) {derivFbar = {0,0,0};}
        ...
        if(stepcount % PROBLEM::OUTINTERVAL == 0) {
            ...
            // std::string filename {"output/ctime_"};
            // filename += std::to_string(ctime);
            // filename += "_.txt";

            //writeArrFile(fieldU, filename);
        }
